...
function assertResolved(asset: AssetReference) { if (isCatalogAssetReference(asset)) throw new Error(`Catalog asset ${asset.catalogKey} was not hydrated before render`); }
export function resolveRawAssetUrl(asset: AssetReference | null | undefined) { if (!asset) return ''; if (typeof asset === 'string') return isAbsoluteUrl(asset) || asset.startsWith('/') ? asset : buildManagedAssetUrl(asset); assertResolved(asset); if (isManagedAssetReference(asset)) return buildManagedAssetUrl(asset.objectKey); return isExternalAssetReference(asset) ? asset.url : ''; }
export function resolveImageUrl(asset: AssetReference | null | undefined, preset: ImagePreset) { if (!asset) return ''; if (typeof asset === 'string') return isAbsoluteUrl(asset) || asset.startsWith('/') ? asset : buildImageUrl(asset, preset); assertResolved(asset); if (isManagedAssetReference(asset)) return buildImageUrl(asset.objectKey, preset); return isExternalAssetReference(asset) ? asset.url : ''; }
export function resolveImagePictureSources(asset: AssetReference | null | undefined, preset: ImagePreset) { if (!asset || typeof asset === 'string') return typeof asset === 'string' && isManagedObjectKey(asset) ? buildImagePictureSources(asset, preset) : null; assertResolved(asset); return isManagedAssetReference(asset) ? buildImagePictureSources(asset.objectKey, preset) : null; }
